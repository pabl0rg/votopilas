...
public class SectionedListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SectionedListAdapter(Context context, int sectionResourceId, int textResourceId,
                                RecyclerView.Adapter baseAdapter, SectionListener listener) {
        mListener = listener;
    ...
    public static class SectionViewHolder extends RecyclerView.ViewHolder {
        ...
        public SectionViewHolder(View view,int mTextResourceid) {
            ...
            hide = (ImageView) view.findViewById(R.id.party_hide_list);
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder sectionViewHolder, final int position) {
        if (isSectionHeaderPosition(position)) {
            ...
            ((SectionViewHolder)sectionViewHolder).hide.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    DialogUtil.simpleDialog(mContext, DialogUtil.HIDE_LIST, new DialogUtil.DialogListener() {
                        @Override
                        public void onClose(boolean isAccepted) {
                            if (isAccepted) {
                                // store in shared preferences
                                UserPreferencesHelper.addToShowLists((String) mSections.get(position).title);
                                mListener.onSectionRemoved(position, position);
                            }
                        }
                    });
                }
    ...
    public interface SectionListener {
        void onSectionRemoved(int start, int end);
    }
}